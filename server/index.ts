...
async function handleGemini(
  ...
  if (!allowed || req.method !== 'POST') {
    sendJson(res, 404, aiError('Unknown AI endpoint'));
    return;
  }
  ...
  try {
    ...
    sendJson(res, 400, aiError('Invalid JSON body'));
    return;
  }
  if (typeof parsed.task !== 'string' || !(allowed as readonly string[]).includes(parsed.task)) {
    sendJson(res, 400, aiError('Invalid task for this endpoint'));
    return;
  }
...
const server = createServer((req, res) => {
  void (async () => {
    ...
    if (req.method !== 'GET' && req.method !== 'HEAD') {
      sendJson(res, 405, aiError('Method not allowed'));
      return;
    }
    await serveStatic(res, url.pathname);
  })().catch((err: unknown) => {
    sendJson(res, 500, aiError(String(err)));
  });
});
